...
static void keyboard_event_cb(lv_event_t *e)
{
    ...
    if (code == LV_EVENT_VALUE_CHANGED) {
        ...
        if (strcmp(txt, LV_SYMBOL_UP) == 0) {
            lv_buttonmatrix_set_map(obj, kb_map_azerty_uc);
        } else if (strcmp(txt, LV_SYMBOL_DOWN) == 0) {
            lv_buttonmatrix_set_map(obj, kb_map_azerty_lc);
        } else if (strcmp(txt, LV_SYMBOL_KEYBOARD) == 0) {
            // Toggle between lowercase, uppercase, and special characters
            if (lv_buttonmatrix_get_map(obj) == kb_map_azerty_lc) {
                lv_buttonmatrix_set_map(obj, kb_map_azerty_uc);
            } else if (lv_buttonmatrix_get_map(obj) == kb_map_azerty_uc) {
                lv_buttonmatrix_set_map(obj, kb_map_azerty_spec);
            } else {
                lv_buttonmatrix_set_map(obj, kb_map_azerty_lc);
            }
        } else if (strcmp(txt, LV_SYMBOL_BACKSPACE) == 0) {
            if (target_textarea) lv_textarea_delete_char(target_textarea);
        } else if (strcmp(txt, LV_SYMBOL_OK) == 0) {
            if (target_textarea) lv_obj_send_event(target_textarea, LV_EVENT_READY, NULL);
...
lv_obj_t *lvgl_keyboard_create(lv_obj_t *parent)
{
    ...
    lv_keyboard_set_map(keyboard, LV_KEYBOARD_MODE_USER_1, kb_map_azerty_lc, NULL); // Set initial keymap to lowercase
